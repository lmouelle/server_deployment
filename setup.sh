...
for container_name in (ls */.config/* | cut -f1 -d/); do
    ...
    chown -R -c luouelle:$container_name $container_name/ --preserve-root
...
groupadd data -f -r -U restic luouelle
chown -Rc --preserve-root luouelle:data /data
chmod -Rc --preserve-root 0760

# Overwrite perms set above for /data/{comics,videos}
...
groupadd torrents -f -r -U deluge restic luouelle
chown -Rc --preserve-root luouelle:torrents /data/torrents 
chmod -Rc --preserve-root 0760
